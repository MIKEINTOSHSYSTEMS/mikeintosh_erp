...
class HrCustody(models.Model):
    ...
    purpose = fields.Char(string='Reason', tracking=True, help="Reason")
    property_ids = fields.One2many('custody.property','custody', string='Property',
                                   help="Property name")
    return_date = fields.Date(string='Return Date', required=False, tracking=True, readonly=True,
                              help="Return date")
    ...
    def check_if_settings_set(self):
        ...
        location_dest_id = int(param.get_param('destination_location'))
        stock_location_id = int(param.get_param('stock_source_location'))
        stock_picking_type_id = int(param.get_param('stock_operation_type'))
        it_location_dest_id = int(param.get_param('it_source_location'))
        it_picking_type_id = int(param.get_param('it_operation_type'))
...
class HrPropertyName(models.Model):
    ...
    qty_onhand = fields.Float(related='product_id.qty_available',readonly=True,string="Available QTY")
